...
void QDirNode::fetchData(QDir search,QStringList &dirs)
{
    ...
    search.setPath(this->path + this->dirPath);
...
void QDirNode::createSubnodes(QStringList dirs, QDir search, QStringList excluded)
{
    ...
    foreach(QString dir, dirs)
    {
        ...
        QDirNode buff = QDirNode(this->path, !this->dirPath.isEmpty() ? this->dirPath + QDir::separator() + dir : dir,search,this->excluded);
...
void QDirNode::getDataPaths(QStringList &pathList)
{
    if(this->fCount)
    {
        ...
        foreach(QString save, this->data)
            pathList << this->path + this->dirPath + QDir::separator() + save;
...
void QDirNode::getFilePaths(QStringList &pathList)
{
    ...
    if(this->fCount)
    {
        // Hier werden alle Inhalte aus dem jetztigen QDirNode in eine QStringList gespeichert.
        foreach(QString save, this->data)
            pathList << this->dirPath + QDir::separator() + save;
...
int QDirNode::getdCount() {
    return this->dCount;
}

int QDirNode::getfCount() {
    return this->fCount;
}

QString QDirNode::getPath() {
    return this->path;
}
/*
